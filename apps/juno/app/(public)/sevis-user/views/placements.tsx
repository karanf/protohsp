        ...
        <CardContent>
          <Metrics metrics={[
            ...
            },
            {
              ...
              icon: CheckCircle,
              variant: "info",
              unit: `${Math.round((placements.filter(p => p.status === 'Confirmed').length / (placements.length || 1)) * 100)}% of total`
            },
            {
              title: "Pending",
              value: placements.filter(p => p.status === 'Pending').length,
              icon: Clock,
              variant: "warning",
              unit: `${Math.round((placements.filter(p => p.status === 'Pending').length / (placements.length || 1)) * 100)}% of total`
            },
            {
              title: "Schools",
              value: new Set(placements.map(p => p.school).filter(Boolean)).size,
              icon: Building2,
              variant: "purple",
